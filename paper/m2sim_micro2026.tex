...
\item Evaluation of 7 PolyBench kernels revealing the in-order vs.\ OoO accuracy gap (136.36\% average error on intermediate benchmarks)
...
\textbf{PolyBench Suite (7 kernels):} Intermediate-complexity linear algebra kernels including matrix-vector operations (MVT, ATAX, BiCG), matrix multiply (GEMM, 2MM, 3MM), and stencil computations (Jacobi-1D). Both simulation and hardware use the SMALL dataset for most kernels; 2MM and 3MM use the MINI dataset. These benchmarks reveal the in-order vs.\ OoO accuracy gap.
...
M2Sim achieves 14.2\% average timing error across 11 microbenchmarks, meeting our sub-20\% accuracy target. Including 7 PolyBench kernels with comparable hardware CPI, the overall average error across 18 benchmarks with error data is 61.71\%, driven by the in-order vs.\ OoO architectural gap on intermediate benchmarks.
...
PolyBench & 7 & 136.4\% & 87.4\% - 263.6\% \\
\midrule
\textbf{Overall (18 benchmarks)} & \textbf{18} & \textbf{61.71\%} & \textbf{1.3\% - 263.6\%} \\
\bottomrule
\end{tabular}
\label{tab:accuracy}
\end{table}
